...
{
    if (empty($input)) {
        echo ERROR("CHECK INPUT FUNCTION: Please fill in all the fields!");
        die();
    } else {
        return $input;
    }
...
//= This function is for granting access to users using session.
function ACCESS_USING_SESSION($name = null, $url = null)
{
...
//= This function is for granting access to users using cookie.
function ACCESS_USING_COOKIE($name = null, $url = null)
{
...
function EXECUTE_SINGLE_ROW_QUERY($query)
{
    global $link;
...
function SELECT_WHERE_ORDER($table = null, $col = null, $data = null, $id = null, $order = null)
{
    ...
    echo "<scrip>alert('Provide accurate arguments for SELECT_WHERE_ORDER')</script>";
}
...
function DELETE_ALL($table = null, $col = null, $data = null)
{
    if ($table != null && $col != null && $data != null) {
        return "DELETE FROM $table WHERE $col = '$data'";
    }
}
...
function GET_TOTAL_FOR_TWO_TABLES($table, $first, $second, $first_param, $second_param, $third_param, $id)
{
    global $link;
...
function GET_TOTAL_WHERE($table = null, $col = null, $data = null)
{
    ...
    if ($table != null && $col != "null" && $data != "null") {
        ...
        if ($execute) {
            ...
            return ERROR("GET TOTAL: Check if <i>" . strtoupper($table) . "</i> table or <i>" . strtoupper($col) . "</i> column matches with what you have in the database");
        }
    }
}
...
//= This function is for checking multiple duplicate data in a specified column from the database
function CHECK_MULTIPLE_DUPLICATE($table, $data, $incoming_data, $data2, $incoming_data2)
{
    global $link;
...
function DECRYPT($old_data, $new_data)
{
    ...
    $new_data = "514318251620wbt" . $new_data;
...
function SEND_MAIL($to, $from, $email, $site, $subject, $msg)
{
    $to = $to;
    $subject = $subject;
...
function SEND_REGISTER_MAIL_CONFIRMATION($to, $from, $username, $email, $site, $subject, $confirm_link)
{
    $to = $to;
    $subject = $subject;
...
//= This function is for getting sessions
function CHECK_SESSION_EXISTS($session_name)
{
    if (!isset($_SESSION[$session_name])) {
        return ERROR("No such session as " . strtoupper($session_name));
    } else {
